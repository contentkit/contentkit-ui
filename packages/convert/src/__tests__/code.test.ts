import { blockConverters } from '@contenkit/convert'
...
it('convertCode', () => {
  const textB = `var x = [1,2,3]`
  const textC = `x.map(y => y + 1)`
  const textD = 'var y;'
  const blocksArray = [
    ...
    new ContentBlock({
      ...
      type: Block.CODE,
      text: textB,
      characterList: List(Repeat(new CharacterMetadata(), textB.length)),
      data: Map({
        language: 'js'
      })
    }),
    new ContentBlock({
      ...
      type: Block.CODE,
      text: textC,
      characterList: List(Repeat(new CharacterMetadata(), textC.length)),
      data: Map({
        language: 'js'
      })
    }),
    new ContentBlock({
      ...
      type: Block.CODE,
      text: textD,
      characterList: List(Repeat(new CharacterMetadata(), textD.length)),
      data: Map({
        language: 'js'
      })
    }),
    new ContentBlock({ key: 'E' }),
    new ContentBlock({ key: 'F' })
  ]
  ...
  expect(
    blockConverters.code(contentState, contentBlock)
  ).toMatchSnapshot()
})
